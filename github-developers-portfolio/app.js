...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("search-form").addEventListener("submit", (e) => {
    ...
    if (query) triggerSearchUsers(query);
  });
  ...
  // Check URL parameters manually for instant search on load (e.g. ?user=octocat or ?q=term)
  checkUrlParams();
});
...
// Manually extract "?user=username" or "?q=query" from URL search query
...
// Search for matching users list (Phase 1)
async function triggerSearchUsers(query) {
  // Sync page URL params for search queries
  const path = window.location.pathname + "?q=" + query;
  ...
  try {
    const data = await fetchGithub("https://api.github.com/search/users?q=" + query);
    const items = data.items;

    const grid = document.getElementById("results-grid");
    grid.innerHTML = "";
    document.getElementById("results-count-title").textContent = data.total_count;

    if (items.length === 0) {
      grid.innerHTML = '<div style="grid-column: 1 / -1; text-align: center; padding: 25px; color: #57606a; font-size: 0.85rem;">No matching users found.</div>';
    } else {
      const limit = items.length > 15 ? 15 : items.length;
      for (let i = 0; i < limit; i++) {
        ...
        card.innerHTML = `
          <img class="search-avatar" src="${item.avatar_url}" alt="${item.login}">
          <div class="search-login">${item.login}</div>
        `;

        card.addEventListener("click", () => {
          runSearch(item.login);
        });
...
// Search developer profile and repos (Phase 2)
async function runSearch(username) {
  // Sync page URL params
