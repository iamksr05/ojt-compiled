...
const knownCategories = ["general", "business", "technology", "sports", "health", "entertainment", "science"];
...
document.querySelector("#category-filters").addEventListener('click', function(e) {
    ...
    if (e.target.getAttribute('data-category')) {
        ...
        document.querySelectorAll("#category-filters .nav-link").forEach(link => {
            link.classList.remove("active");
        });
        e.target.classList.add("active");
        
        // Reset dropdown choice
        document.getElementById("dropdown-category-filter").value = "";
...
document.querySelector("#dropdown-category-filter").addEventListener('change', function(e) {
    if (e.target.value != "") {
        ...
        // Sync active category filters menu
        document.querySelectorAll("#category-filters .nav-link").forEach(link => {
...
async function Fetchnews(category) {
    ...
    try {
        ...
        // DYNAMIC SWITCH: Route valid categories to the stable category pool (Saurav API).
        // Otherwise, treat as keyword search term and query the search engine (Spaceflight News API).
